...
<div class="row clearfix">
    ...
    <div class="card">
            ...
            <ul class="nav nav-tabs">
                @if(count(\App\Models\VendorManagementgiinit::select('created_at')->where('id_supplier', $this->selected_id)->get()) < 1)
                
                <li class="nav-item"><a class="nav-link active show" data-toggle="tab" href="#newevaluation">New Evaluation</a></li>
                @else
                    @foreach(\App\Models\VendorManagementgiinit::select('created_at')->where('id_supplier', $this->selected_id)->groupBy(DB::Raw('date(created_at)'))->orderBy(DB::Raw('date(created_at)'), 'desc')->get() as $key => $item)
            ...
            <div class="tab-content">
                <!-- if(!\App\Models\VendorManagementgiinit::where('id_supplier', $this->selected_id)->get()) -->
                @if(count(\App\Models\VendorManagementgiinit::select('created_at')->where('id_supplier', $this->selected_id)->get()) < 1)
                                ...
                                <form wire:submit.prevent="save">
                                    ...
                                    <div class="col-md-12">
                                        <div style="border: 1px solid lightgrey; border-radius: 5px; padding: 10px; margin: 5px;">
                                            <?php
                                                $check_data = \App\Models\VendorManagementgiinit::where('id_supplier', $this->selected_id)->get();
                                            ?>
                ...
                @else
                    @foreach(\App\Models\VendorManagementgiinit::select('created_at')->where('id_supplier', $this->selected_id)->groupBy(DB::Raw('date(created_at)'))->orderBy(DB::Raw('date(created_at)'), 'desc')->get() as $item)
                    <div class="tab-pane  active show" id="historigi<?php echo date_format(date_create($item->created_at), 'dMY'); ?>">
                        @livewire('vendor-management.historiinitgeneralinformation', ['date' => $item->created_at, 'selected_id' => $this->selected_id])
                    </div>
                    @endforeach
                @endif    
            </div>
