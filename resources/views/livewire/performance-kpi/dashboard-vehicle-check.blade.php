                ...
                @foreach($region_project as $key_region => $region)
                    ...
                    @foreach($sub_region as $key_sub =>  $sub)
                        <tr>
                            ...
                            @endif
                            @php($done = \App\Models\VehicleCheck::where(['is_submit'=>1,'client_project_id'=>$p->id,'region_id'=>$region->region_id,'sub_region_id'=>$sub->region_cluster_id])->count())
                            @php($not_done = \App\Models\VehicleCheck::where(['is_submit'=>0,'client_project_id'=>$p->id,'region_id'=>$region->region_id,'sub_region_id'=>$sub->region_cluster_id])->count())
                            @php($sticker_safety = \App\Models\VehicleCheck::where(['stiker_safety_driving'=>0,'client_project_id'=>$p->id,'region_id'=>$region->region_id,'sub_region_id'=>$sub->region_cluster_id])->count())
