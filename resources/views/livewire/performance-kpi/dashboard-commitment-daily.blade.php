        ...
        <thead>
            <tr style="background:#eee;">                 
                ...
                @foreach(config('vars.commitment_daily_statement') as $field)
                    <th colspan="4" class="text-center">{{$field}}</th>
                @endforeach
            </tr>
            <tr style="background:#eee;">
                @foreach(config('vars.commitment_daily_statement') as $field)
                    ...
                    @foreach($sub_region as $key_sub => $sub)
                        <tr>
                            ...
                            @foreach(config('vars.commitment_daily_statement') as $key => $field)
                                @php($count = \App\Models\CommitmentDaily::where(['client_project_id'=> $p->id,'region_id'=>$region->region_id,'sub_region_id'=>$sub->region_cluster_id])->whereIn($key,[1,0])->get())
                                
                                @php($done = \App\Models\CommitmentDaily::where(['client_project_id'=> $p->id,'region_id'=>$region->region_id,'sub_region_id'=>$sub->region_cluster_id])->where($key,1)->count())
                                @php($not_done = \App\Models\CommitmentDaily::where(['client_project_id'=> $p->id,'region_id'=>$region->region_id,'sub_region_id'=>$sub->region_cluster_id])->where($key,0)->count())
