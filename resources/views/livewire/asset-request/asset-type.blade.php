<form wire:submit.prevent="save">
    <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fa fa-database"></i> Asset Type</h5>
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach($data as $k =>  $item)
                        <tr>
                            <td>{{$k+1}}</td>
                            <td>@livewire('asset-request.asset-request-editable',['data'=>$item,'field'=>'asset_type'],key($item->id))</td>
                            <td></td>
                        </tr>
                    @endforeach
                    @if($insert)
                        <tr>
                            <td></td>
                            <td>
                                <input type="text" class="form-control" wire:model="asset_type" placeholder="Name" />
                                @error('name')
