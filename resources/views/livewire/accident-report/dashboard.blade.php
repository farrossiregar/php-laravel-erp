        ...
        <div class="col-md-2" wire:ignore>
            <select class="form-control" style="width:100%;" wire:model="month">
                ...
                <?php
                    for($i=1; $i<=12; $i++){
                ?>
                <option value="<?php echo $i; ?>">{{date('F', mktime(0, 0, 0, $i, 10))}}</option>
                <?php
                    }
                ?>
            </select>
...
$( document ).ready(function() {
    ...
    //     }
    // );
    init_chart_accidentreport();
});
Livewire.on('init-chart-acc',(data)=>{
    labels = JSON.parse(data.labels);
    datasets = JSON.parse(data.datasets);
    init_chart_accidentreport();
});
function init_chart_accidentreport(){
    var colors = ['#007bff','#28a745','#333333','#c3e6cb','#dc3545','#6c757d'];
