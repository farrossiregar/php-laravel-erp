...
class Submitfinreg extends Component
{
    ...
    public function save()
    {
        ...
        $this->file->storeAs("public/po-tracking-nonms/{$data->id}", $name);
        $transfer->file = "storage/po-tracking-nonms/{$data->id}/{$name}";
        $transfer->amount = $this->amount;
        $transfer->po_tracking_nonms_master_id = $data->id;
        $transfer->save();
