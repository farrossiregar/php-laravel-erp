...
class Importsettlement extends Component
{
    protected $listeners = [
        'modalimportsettlement'=>'modalimportsettlement',
    ];

    use WithFileUploads;
    public $file, $selected_id, $settlement, $unused_amount;

    
    public function render()
    {
        return view('livewire.consumable-item-database.importsettlement');
        
    }

    public function modalimportsettlement($id)
    {
        $this->selected_id = $id;
        $this->settlement = \App\Models\ConsumableItemRequest::where('id', $this->selected_id)->first()->settlement;
    }
    ...
    public function save()
    {
        ...
        if($this->file){
            $settlement = 'settlement'.$this->selected_id.'.'.$this->file->extension();
            $this->file->storePubliclyAs('public/Consumable_Item_Database/',$settlement);

            $data                           = \App\Models\ConsumableItemRequest::where('id', $this->selected_id)->first();
            $data->settlement               = $settlement;
            $data->unused_amount            = $this->unused_amount;
            if($this->unused_amount && $this->unused_amount != '0'){
                $data->return_dana_pettycash    = $this->unused_amount * $data->price;
            }else{
                $data->return_dana_pettycash    = '';
            }
        ...
        session()->flash('message-success',"Upload Settlement Item Request Success!!!");
