...
class Importapprovedverificationdocs extends Component
{
    ...
    public function save()
    {
        ...
        if($this->file){
            $appverdocs = 'poms-appverdocs'.$this->selected_id.'.'.$this->file->extension();
            $this->file->storePubliclyAs('public/po_tracking_ms/Approval_verification_docs/',$appverdocs);

            $data = \App\Models\PoTrackingMs::where('id', $this->selected_id)->first();
            $data->approved_verification         = $appverdocs;
