...
{
    public $date_start,$date_end,$labels,$series,$total_submitted,$total_approved_eid,$total_pm;
    ...
    public function init_data()
    {
        $data = PreventiveMaintenance::select("pm2.*",
                                ...
                                \DB::raw("(SELECT count(*) FROM preventive_maintenance pm where pm.site_type=pm2.site_type and pm.pm_type=pm2.pm_type and pm.region_id=pm2.region_id and pm.sub_region_id=pm2.sub_region_id and DATE(pm.created_at)=DATE(CURRENT_DATE()) and pm.status=2 and is_upload_report=1) as approved_ied")
                            )
                                ->from('preventive_maintenance','pm2')
                            ->with(['region','sub_region'])->groupBy('region_id','sub_region_id','site_type','pm_type');
        $total_submitted = PreventiveMaintenance::where(['status'=>2]);
    ...
    public function chart()
    {
        ...
        $data = PreventiveMaintenance::with(['region','sub_region'])->groupBy('region_id','sub_region_id')->get();
        ...
        foreach(['SOW (Monthly Target)','Submitted','Approved IED'] as $k=>$item){
