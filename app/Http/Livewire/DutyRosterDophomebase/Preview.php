...
class Preview extends Component
{
    use WithPagination;
    public $data, $data_id;
    public $region, $project, $nama_dop, $selected_id, $te, $cme;
    protected $paginationTheme = 'bootstrap';
    
    public function render()
    {
        $data = \App\Models\DutyrosterDophomebaseDetail::where('id_master_dutyroster', $this->selected_id); 
        ...
        return view('livewire.duty-roster-dophomebase.preview');
    ...
    public function mount($id){
        ...
        foreach(\App\Models\DutyrosterDophomebaseDetail::where('id_master_dutyroster', $id)->where('remarks', '1')->get() as $item){
    ...
    {
        $check = \App\Models\DutyrosterDophomebaseDetail::where('id',$id)->first();
        if($check->remarks == '1'){
            $check->remarks = '';
        }else{
            $check->remarks = '1';
    ...
    public function save($id)
    {
        ...
        $data = \App\Models\DutyrosterDophomebaseDetail::where('id_master_dutyroster', $id)
                                        ->orderBy('id', 'asc')
                                        ->get();
        ...
        foreach($data as $k => $item){
            $objPHPExcel->setActiveSheetIndex(0)
                            ...
                            ->setCellValue('K'.$num, 'expired')
                            ->setCellValue('L'.$num, 'budget');
        ...
        header('Content-Disposition: attachment;filename="Report-Dutyroster-Dophomebase.xlsx"');
        ...
        },'Report-Dutyroster-Dophomebase.xlsx');
