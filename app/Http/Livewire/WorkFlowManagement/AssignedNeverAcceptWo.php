...
class AssignedNeverAcceptWo extends Component
{
    ...
    public function generate_chart()
    {
        ...
        })->groupBy('region_dan_asp_info','skills')->get() as $k => $item)
        {
            ...
            {
                $this->series[$k]['data'][$key_data] = \App\Models\WorkFlowManagement::where(['date'=>$data->date,'region_dan_asp_info'=>$data->region_dan_asp_info,'skills'=>$data->skills,'wo_accept'=>0])->where('wo_assign','!=',0)->count();
            }
        }
        $this->labels = json_encode($this->labels);
        $this->series = json_encode($this->series);
        $this->emit('chart-assigned-never-accept-wo',['labels'=>$this->labels,'series'=>$this->series]);   
    }
}