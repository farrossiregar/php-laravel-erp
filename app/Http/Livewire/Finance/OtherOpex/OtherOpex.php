...
class OtherOpex extends Component
{
    ...
    public function render()
    {
        ...
        $data = AccountPayableOtheropex::select('account_payable_otheropex.*', 'account_payable.subrequest_type')
                                        ->join('account_payable','account_payable.id','=','account_payable_otheropex.id_master')
                                        ->where('account_payable.request_type','3')
                                        ->where('account_payable_otheropex.company_id',session()->get('company_id'))
                                        ->orderBy('account_payable_otheropex.updated_at','DESC');
        ...
        return view('livewire.finance.other-opex.other-opex')->with(['data'=>$data->paginate(100)]);
    }
